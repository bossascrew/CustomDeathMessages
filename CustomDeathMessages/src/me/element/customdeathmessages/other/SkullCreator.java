...
public class SkullCreator {
	 ...
	 * @return The head of the Player.
	 */
	public static ItemStack itemFromUuid(UUID id, CustomDeathMessages plugin) {
		return itemWithUuid(createSkull(), id, plugin);
	}
	 ...
	 * @return The head of the Player.
	 */
	public static ItemStack itemWithUuid(ItemStack item, UUID id, CustomDeathMessages plugin) {
		notNull(item, "item");
		notNull(id, "id");

		SkullMeta meta = (SkullMeta) item.getItemMeta();
		
		if (plugin.getServerVersion().getVersionInt() >= VersionEnums.VERSION_113.getVersionInt())
		{
	...
	private static void checkLegacy() {
		try {
			...
			if (!warningPosted) {
				Bukkit.getLogger().info("[CustomDeathMessages] Modern API versions may have issues with player skulls. (Should work as expected however)");
				warningPosted = true;
			}
